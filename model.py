...
class Encoder_content_reduceplusplus(nn.Module):
    def __init__(self, n_channels = 1, n_classes = 1, n_features = 64):
        super(Encoder_content_reduceplusplus, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.n_features = n_features
        ...
        for i in range(1,2):
            encoder_LR+=[ReLUINSConv2d(self.n_features, self.n_features * 4, kernel_size=3, stride=2, padding=1)]
            encoder_HR+=[ReLUINSConv2d(self.n_features, self.n_features * 4, kernel_size=3, stride=2, padding=1)]
...
class Discriminateur_content_reduceplus(nn.Module):
    def __init__(self, n_features = 256):
        super(Discriminateur_content_reduceplus, self).__init__()
        self.n_features=n_features
        self.model=nn.Sequential(
...
class SupervisedContrastiveLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        if labels is not None and mask is not None:
            ...
            # mask = torch.eq(labels, labels.T).float().to(torch.device("cuda:"+str(self.gpu)))
            mask = torch.eq(labels, labels.T).float().to(self.Device)
        else:
            # mask = mask.float().to(torch.device("cuda:"+str(self.gpu)))
...
class Loss_DRIT_custom_last_version(nn.Module):
    ...
    def backward_EG(self, x, y, fake_LR, fake_HR, fake_LRLR, fake_HRHR, est_LR, est_HR, z_LR, z_HR, content_LR, content_HR, z_fake_LR, z_fake_HR, content_fake_LR, content_fake_HR, discriminator_content, discriminator_LR, discriminator_HR):
        ...
        # KL loss - z_a
        loss_kl_za_LR = self._l2_regularize(z_LR) * self.lambda_reg 
        loss_kl_za_HR = self._l2_regularize(z_HR) * self.lambda_reg 
    ...
    def forward(self, manual_backward, log, optimizer, LR, HR, fake_LR, fake_HR, fake_LRLR, fake_HRHR, est_LR, est_HR, z_LR, z_HR, content_LR, content_HR, z_fake_LR, z_fake_HR, content_fake_LR, content_fake_HR, z_random, fake_LR_random, fake_HR_random, z_random_LR, z_random_HR, LR_random, HR_random, discriminator_content, discriminator_LR, discriminator_HR, anatomy_encoder, modality_encoder, encode_content, encode_LR, encode_HR, generator_LR, generator_HR, discriminator_LR2=None, discriminator_HR2=None, segmentation=None, segmentation_predict=None, segmentation_net=None):
        ...
        if self.random_size!= 0 and self.use_segmentation_network:
            ...
            optimizer_idx = 8
            # print("optimizer idx: ", optimizer_idx)
...
class mDRITpp(pl.LightningModule):
    ...
    def __init__(self, opt, prefix, isTrain):
        ...
        if isTrain:
            self.Loss = Loss_DRIT_custom_last_version(opt, gpu=self.gpu)       
    ...
    def forward(self, x, y):
        ...
        if self.use_segmentation_network:
            segmentation_HR = self.segmentation_net(content_HR)
            segmentation_LR = self.segmentation_net(content_LR)
            return(fake_HR, fake_LR, segmentation_HR, segmentation_LR)
        elif self.mode=='reconstruction':
            return(fake_HR)
        elif self.mode=='degradation':
            return(fake_LR)
        else:
            sys.exit("Please indicate the appropriate mode for inference: reconstruction or degradation.")
    ...
    def get_latent(self, x, y):
        ...
        return(x, y, content_HR, content_LR, z_HR, z_LR, z_fake_HR, z_fake_LR, content_fake_HR, content_fake_LR)
